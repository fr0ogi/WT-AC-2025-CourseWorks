...
- POST `/tickets` — создать заявку (например, `request_device` или `request_home`)
  ...
  ```json
  {
    "type": "request_device",
    "requesterId": "user-1",
    "deviceInfo": { "name": "Уличный датчик", "description": "..." }
  }
  ```
...
- AC-T1 (Create): POST /tickets с {queueId, title, message?} создаёт тикет: isClose=false, agent=null. Если есть message — добавляется первое сообщение с from="user".
- AC-T2 (Scope list): GET /tickets
  user → только свои; agent → только назначенные; admin → все.
- AC-T3 (Get one): GET /tickets/{id} доступен владельцу, назначенному агенту или админу.
- AC-M1 (Add message): POST /tickets/{id}/messages с {text} добавляет в messages запись с from="user"| "agent" по роли автора. Пустой text запрещён.
- AC-M2 (Closed guard): если ticket.isClose=true, добавлять сообщения нельзя → 409
- AC-A1 (Assign agent): POST /tickets/{id}/assign с {agentId} (только admin) назначает агента, если открытых тикетов у агента < agent.capacity.
- AC-A2 (Reassign): повторный assign меняет агента, если условие вместимости нового агента выполнено.
- AC-C1 (Close ticket): POST /tickets/{id}/close ставит isClose=true. Кто может: владелец тикета, назначенный агент, admin. Идемпотентно.
- AC-C2 (Post-close): после закрытия любые POST /tickets/{id}/messages → 409
- AC-R1 (Create rating): POST /ratings с {ticketId, score, comment?}:
  допускается, если ticket.isClose=true, у тикета есть agent, автор — владелец тикета (или admin от его имени); score ∈ [1..5]; один отзыв на тикет от пользователя (уникальность (ticketId,userId)).
- AC-R2 (Scope list): GET /ratings user → только свои; agent → только свои входящие (по себе); admin → все. Фильтры: ?agentId=, ?ticketId=.
